...
| Keep a row each time a modifier is released | When on, releasing part of a combination and **holding the rest for more than 0.7 seconds** leaves the combination so far as history, and the keys still held appear as a new row (⇧⌘ → ⇧). Release everything sooner and it stays a single row, so ordinary shortcuts never add rows. Useful when the press-and-release steps are what you are teaching (default off) |
