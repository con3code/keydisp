...
| Keep a row each time a modifier is released | When off (default), pressing ⌘⇧ and releasing them one by one stays a single "⇧⌘" row. When on, each release leaves the combination so far as history and the keys still held appear as a new row (⇧⌘ → ⇧) — useful when the press-and-release steps are what you are teaching |
...
Shows the status of Accessibility and Input Monitoring.
...
3. Check Settings → Permissions: both should show "Granted"
